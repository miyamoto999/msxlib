/*********************************************************
 bfile/bfile_read_dsk1.c mk_dskcallversion.sh によって自動生成されるファイルなので
 書き換えないでください。

  Do not rewrite bfile/bfile_read_dsk1.c as this file is
...
int16_t __LIB__ bfile_read_dsk1(BFILE_DOS1 *fp, void *buf, int16_t size) __smallc
{
    ...
    if(data_size >= size) {
        ...
        fp->buf_offset = 0;
        fp->dirty = TRUE;
        data_size += bfile_read_dsk1(fp, &buf[data_size], size - data_size);
    }
    return data_size;
}
